...
async function runMorningJob({ force = false } = {}) {
  ...
  for (const t of tokens) {
    ...
    if (dayType === 'friday') {
      ...
      subject = '[점심] 오늘 점심 참석 여부를 알려주세요 (마감: 오후 2:29)';
...
function startScheduler() {
  // 테스트 설정: 14:20 초대(강제 초기화), 14:30 결과 (응답 마감 14:29)
  cron.schedule('20 14 * * 1-5', () => runMorningJob({ force: true }), { timezone: 'Asia/Seoul' });
  cron.schedule('30 14 * * 1-5', runResultsJob, { timezone: 'Asia/Seoul' });
  console.log('[scheduler] 스케줄러 시작됨 (테스트: 14:20 초대[강제], 14:30 결과)');
}
