...
async function runMorningJob({ force = false } = {}) {
  ...
  console.log(`[scheduler] 오전 9시 잡 실행: ${today}`);
  ...
  for (const t of tokens) {
    ...
    if (dayType === 'friday') {
      ...
      subject = '[점심] 오늘 점심 참석 여부를 알려주세요 (마감: 오전 10:59)';
...
async function runResultsJob() {
  const today = getTodaySeoul();
  console.log(`[scheduler] 오전 11시 잡 실행: ${today}`);
...
function startScheduler() {
  cron.schedule('0 9 * * 1-5', runMorningJob, { timezone: 'Asia/Seoul' });
  cron.schedule('0 11 * * 1-5', runResultsJob, { timezone: 'Asia/Seoul' });
  console.log('[scheduler] 스케줄러 시작됨 (9시 초대, 11시 결과)');
}
