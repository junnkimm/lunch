...
async function runMorningJob({ force = false } = {}) {
  ...
  for (const t of tokens) {
    ...
    if (dayType === 'friday') {
      ...
      subject = '[점심] 오늘 점심 참석 여부를 알려주세요 (마감: 오후 1:59)';
...
function startScheduler() {
  // 테스트 설정: 13:50 초대, 14:00 결과 (응답 마감 13:59)
  cron.schedule('50 13 * * 1-5', runMorningJob, { timezone: 'Asia/Seoul' });
  cron.schedule('0 14 * * 1-5', runResultsJob, { timezone: 'Asia/Seoul' });
  console.log('[scheduler] 스케줄러 시작됨 (테스트: 13:50 초대, 14:00 결과)');
}
