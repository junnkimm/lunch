...
async function runMorningJob({ force = false } = {}) {
  ...
  for (const t of tokens) {
    ...
    if (dayType === 'friday') {
      ...
      subject = '[점심] 오늘 점심 참석 여부를 알려주세요 (마감: 오후 3:59)';
...
function startScheduler() {
  // 테스트 설정: 초대는 배포 후 수동 트리거, 결과는 16:00 (응답 마감 15:59)
  cron.schedule('0 16 * * 1-5', runResultsJob, { timezone: 'Asia/Seoul' });
  console.log('[scheduler] 스케줄러 시작됨 (테스트: 초대 수동, 16:00 결과)');
}
